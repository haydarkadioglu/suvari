...
def help():
    ...
    t.add_row("configure", "Interactive setup — provider, model, API key")
    t.add_row("", "")
    t.add_row("scan <url>", "Full scan — tree-based recon → analysis → report")
    ...
    t.add_row("", "  -p, --provider   LLM provider")
    t.add_row("", "  -m, --model      Model name override")
    t.add_row("", "")
    t.add_row("recon <url>", "Quick reconnaissance only")
    t.add_row("attack <dir>", "Exploit findings from previous scan")
    t.add_row("bb <url>", "Bug bounty recon — subdomains, URLs, params")
    t.add_row("chat", "Interactive chat + CTF problem solving")
    t.add_row("report <dir>", "Show scan report")
    t.add_row("list", "List previous scans")
    t.add_row("help", "Show this message")
    t.add_row("", "")
    t.add_row("MCP server:", style="bold yellow")
    t.add_row("suvari_mcp.py", "Run as MCP server for Claude/Cursor")
    t.add_row("", "  Tools: scan_target, recon_target, run_tool,")
    t.add_row("", "         list_available_tools, get_scan_report, analyze_ctf")
    t.add_row("", "")
    t.add_row("Examples:", style="bold green")
    t.add_row("", "python suvari.py scan https://example.com")
    t.add_row("", "python suvari.py scan https://server.com -s -P 5")
    t.add_row("", "python suvari.py scan https://app.com -r ./src")
    t.add_row("", "python suvari.py chat")
    t.add_row("", "python suvari.py attack output/20260101_*/")
    console.print(t)
